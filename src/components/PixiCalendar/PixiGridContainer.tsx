...
export const PixiGridContainer: React.FC<GridContainerProps> = ({
  ...
  const renderWindowEndDate = useMemo(() => 
    visibleEndDate.add(CALENDAR.RENDER_WINDOW, "day"),
    [visibleEndDate]
  );
  ...
  const drawCells = useCallback(
    (g: Graphic) => {
      ...
      // Csak a renderelési ablakban lévő cellákat rajzoljuk ki
      ...
      groups.forEach((_, index) => {
        for (let i = startDayIndex; i <= endDayIndex; i++) {
          ...
          // Right border
          g.moveTo(x + CALENDAR.GRID_WIDTH, y);
          g.lineTo(x + CALENDAR.GRID_WIDTH, y + CALENDAR.GRID_HEIGHT);
          
          // Bottom border
          g.moveTo(x, y + CALENDAR.GRID_HEIGHT);
          g.lineTo(x + CALENDAR.GRID_WIDTH, y + CALENDAR.GRID_HEIGHT);
          
          // Top border only for first row
          if (isTopRow) {
            g.moveTo(x, y);
            g.lineTo(x + CALENDAR.GRID_WIDTH, y);
          }
        }
      });
    },
    [groups, totalRenderDays, renderRange.startDate, renderWindowStartDate, renderWindowEndDate, today]
  );
